...
const envSchema = z.object({
  ...
  X402_PRICE: z.string().regex(/^\d+$/, "X402_PRICE must be atomic units (decimal integer)").default("10000"),
  X402_BATCH_PRICE: z.string().regex(/^\d+$/, "X402_BATCH_PRICE must be atomic units (decimal integer)").default("50000"),
...
export function getNotaryConfig(): NotaryConfig | null {
  ...
  return {
    ...
    batchPrice: BigInt(env.X402_BATCH_PRICE),
